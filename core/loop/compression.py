"""CompressionService：L1/L2 上下文压缩策略。
...
注：本服务不再做 L3 落 Memory 操作。Memory.md 的写入由 LLM 通过 Bash 直接进行
（见 system prompt 中的 `## Memory` section），属于低频、显式行为。
"""
...
from core.protocol import LLMProxy, ToolResult
...
class CompressionService:
    ...
    async def maybe_summarize(
        ...
        """
        new, _, _ = await self._try_summarize(messages)
        return new
    ...
    async def summarize_for_trace(
        ...
        """
        return await self._try_summarize(messages)
